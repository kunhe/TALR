function [net, stats] = train_imagenet(net, imdb, trainBatch, testBatch, varargin)
...
start = min(start, opts.numEpochs);
if start >= 1 %&& start <= opts.numEpochs
    fprintf('%s: resuming by loading epoch %d\n', mfilename, start) ;
    opts_ = net.layers{end}.opts;
    [net, state, stats] = loadState(modelPath(start)) ;
    net.layers{end}.opts = opts_;
    disp(net.layers{end})
else
    state = [] ;
end
...
for epoch=start+1:opts.numEpochs
    ...
    params.imdb = imdb ;
    params.getBatch = trainBatch ;

    if numel(params.gpus) <= 1
        [net, state] = processEpoch(net, state, params, 'train', hasError) ;
        [net, state] = processEpoch(net, state, params, 'val', hasError) ;
    ...
    clear lastStats ;
    if ismember(epoch, opts.saveEpochs)
    ...
    % [KH]
    if ~isempty(opts.epochCallback)
        net = opts.epochCallback(epoch, net, imdb, testBatch);
    end
end
...
% -------------------------------------------------------------------------
function [net, state] = processEpoch(net, state, params, mode, hasError)
% -------------------------------------------------------------------------
...
dclck = tic ;
objs  = [];
myLogInfo('Epoch %d, learningRate = %d', params.epoch, params.learningRate);
for t=1:params.batchSize:numel(subset)
    ...
    for s=1:params.numSubBatches
        ...
        % debug
        if params.debug & strcmp(mode, 'train') & rand>0.99
            res = vl_simplenn(net, im, dzdy, res);
    ...
    if t == 1 || toc(dclck) > 3 || numel(subset)-t <= params.batchSize
        fprintf('%s: epoch %02d: %3d/%3d:', mode, params.epoch, ...
            fix((t-1)/params.batchSize)+1, ceil(numel(subset)/params.batchSize)) ;
        fprintf(' %.1f Hz', averageSpeed) ;
...
myLogInfo('Epoch %d, Avg %s objective = %g', params.epoch, mode, mean(objs));
end  % if N > 0
